...
import 'package:login_flutterbook_provider/utils.dart' as utils;
...
class NotesDBWorker {
  ...
  /// static final 변수로서 외부 파일에서 Notes의 DB에 관한 것을 다룰 때는 이 인스턴스
  /// 로 계속 진행함. static 변수이므로 처음 컴파일부터 끝까지 메모리를 잡고 유지됨.
  /// final 이므로 중간에 수정되려면 아래 메서드들을 거쳐야 함.
  ...
  Future<Database> init() async {
    ...
    print("## utils.docsDir.path: ${utils.docsDir.path}");
    ...
    Database db = await openDatabase(
      path,
      version: 1,
      /// function that is called when a file is open
      onOpen: (db) {},
      /// function that is called when a file is newly created.
      /// In this case,
